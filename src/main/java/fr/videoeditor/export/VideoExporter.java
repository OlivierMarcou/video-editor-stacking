        ...
        SwingWorker<Boolean, String> worker = new SwingWorker<>() {
            ...
            protected Boolean doInBackground() throws Exception {
                ...
                try {
                    ...
                    // Créer l'enregistreur AVI avec codec FFV1 (lossless)
                    recorder = new FFmpegFrameRecorder(outputFile, width, height, audioChannels);
                    recorder.setVideoCodec(avcodec.AV_CODEC_ID_FFV1); // FFV1 = codec sans perte
                    recorder.setFormat("avi");
                    recorder.setFrameRate(frameRate);

                    // Options pour codec sans perte
                    recorder.setVideoOption("level", "3");
                    recorder.setVideoOption("coder", "1");
                    recorder.setVideoOption("context", "1");

                    // Audio sans perte (PCM)
                    if (audioChannels > 0) {
                        recorder.setAudioCodec(avcodec.AV_CODEC_ID_PCM_S16LE); // PCM 16-bit sans perte
                    ...
                    publish("Codec: FFV1 (lossless), Audio: PCM");
                    ...
                    for (int segIdx = 0; segIdx < segments.size(); segIdx++) {
                        ...
                        // Traiter ce segment (même logique que MP4)
                        FFmpegFrameGrabber grabber = new FFmpegFrameGrabber(segment.getVideoFile());
                        grabber.start();
